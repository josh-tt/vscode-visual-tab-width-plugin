...
export function activate(context: vscode.ExtensionContext) {
	...
	function applyDecorations() {
		if (!isWiderTabsActive || !decorationType) return;
	...
	function toggleWiderTabs() {
		...
		vscode.window.showInformationMessage(
			`Wider tabs display: ${isWiderTabsActive ? "ON" : "OFF"}`
		);
	}
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(
			"visualTabWidth.showOptions",
			showOptions
		),
		vscode.commands.registerCommand(
			"visualTabWidth.toggleWiderTabs",
			toggleWiderTabs
		),
		vscode.commands.registerCommand(
			"visualTabWidth.hideIndents",
			hideIndents
		),
		vscode.commands.registerCommand(
			"visualTabWidth.showIndents",
			showIndents
		),
